...
void OnMouseExit()
       ...
       void OnMouseDown()
    {
        if (activeCell)
        {
            ...
            gameObject.transform.parent.gameObject.GetComponent<CellControl>().OnTriggerEnter1(tower.transform.GetChild(0).GetComponent<Collider>());
            if (gameObject.transform.parent.gameObject.GetComponent<CellControl>().allowPut)
            {
                Destroy(UIControll.instance.chosentower);
